...
- AIに並列でコーディングさせようとしたが、人間パート（レビューやプロンプトの入力や動作確認）でストップされるので、そんなに速度が上がっている感じがしなくて直列に戻した。
  - 多少質が落ちてもいいので速いモデルがほしい。CursorのComposer 2が気になる。
  - 少し進めてみて最低限動くものはできそうだったので、中終盤はコードレビューしていない。動作確認のみ。
- 動画作成は動画編集ソフトを触らなくていいのが良かった。
